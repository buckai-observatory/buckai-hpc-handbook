...
```sshconfig
### Template for a compute node — replace `mynode` with the real
### hostname of a node you've been allocated.
Host mynode
...
You can use VS Code Remote-SSH to connect to a compute node directly: in the **Remote-SSH: Connect to Host** dialog, type your node alias (e.g. `mynode`) instead of `unity`. This gives you the **full** VS Code experience — editor, integrated terminal, Claude Code — running on the compute node, with **no port forwarding ceremony**.
