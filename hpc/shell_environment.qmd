...
✔ Type `getgpu` instead of `sinteractive -p mygroup -t 06:00:00 -g 1 -w u126`
...
The annoying part: by default `.bash_profile` does **not** run `.bashrc`. So if you put your aliases in `.bashrc`, an SSH session won't see them.

### The fix: source `.bashrc` from `.bash_profile`

Put this at the bottom of your `~/.bash_profile`:
...
# Forward Jupyter from a GPU node to localhost:9099
...
alias getgpu='sinteractive -p mygroup -t 06:00:00 -g 1 -w u126'
alias get170='sinteractive -p mygroup -w u170 -c 48 -t 12:00:00'
...
You'd call it as `slinfo 5067189` — `"$1"` is replaced with `5067189` at runtime.
...
| `ANTHROPIC_API_KEY` | Used by Claude Code (see warning below) |
...
| `umask` | Default file/directory permissions (Section 6) |
...
# 5. `module load`: The Cluster's Software Toolkit
...
See the [Modules](modules.qmd) page for more.
...
# 7. The Conda/Mamba Initialization Block
...
# 8. Putting It All Together: An Annotated Example `.bashrc`

This is a realistic, sanitized example showing common patterns. Replace `<username>` and `<group>` with your own values, and **do not paste tokens** straight into a `.bashrc` — see Section 4.
...
alias getnode='sinteractive -m 48000 -t 48:00:00'
alias getgpu='sinteractive -p <group> -t 06:00:00 -g 1 -w u126'
alias get170='sinteractive -p <group> -w u170 -c 48 -t 12:00:00'
...
# ───────────────────────────────────────────────────────────────────────
# (block omitted for brevity — see Section 7)
